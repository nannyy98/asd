...
def format_date(date_string):
    ...
    try:
        ...
        print(f"Ошибка форматирования даты {date_string}: {e}")
        return date_string
...
def create_product_card(product):
    ...
    product_id, name, description, price, category_id, image_url, stock, views = product[0:8]
    
    card = f"<b>{escape_html(name)}</b>\n\n"
    
    if description:
        card += f"{escape_html(truncate_text(description, 150))}\n\n"
    
    card += f"💰 Цена: <b>{format_price(price)}</b>\n"
    card += f"📦 В наличии: {stock} шт.\n"
    card += f"👁 Просмотров: {views}\n"
...
def log_user_action(telegram_id, action, details=""):
    ...
    print(f"[{timestamp}] User {telegram_id}: {action} {details}")
